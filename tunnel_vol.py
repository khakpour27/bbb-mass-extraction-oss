...
logging.info("Processing tunnel volumes from %s", target_gdb)

# Check if tunnel data exists
if not arcpy.Exists(tunnel_mp):
    logging.info("No tunnel data found in %s, skipping tunnel volume calculation", target_gdb)
    tunnel_vol = 0
    tunnel_vekt = 0
else:
...
logging.info("Tunnel volume: %.1f m3, weight: %.1f kg", tunnel_vol, tunnel_vekt)
...
logging.info("Updated %s with tunnel volumes", target_xlsx)
arcpy.CheckInExtension("3D")
arcpy.CheckInExtension("Spatial")
logging.info("tunnel_vol.py complete")
