"""
publish_legacy.py — Publishes legacy pipeline results to AGOL for comparison.

Publishes to folder "Parametrisk masseuttak (legacy)" with _legacy suffix.
...
AGOL_FOLDER = "Parametrisk masseuttak (legacy)"
ITEM_SUFFIX = "_legacy"
...
print(f"Publishing legacy results from: {latest_output_fld_path}")
...
footprint_props = ItemProperties(
    ...
    description="Fotavtrykk for gravemodellen (legacy pipeline) — kun for sammenligning",
    item_type=ItemTypeEnum.FILE_GEODATABASE,
    tags=["BB5", "Parametrisk masseuttak", "legacy"]
)
...
elev_result, elev_package_item, elev_publishing_result = arcpy.management.SharePackage(
    ...
    username="",
    password="",
    summary=f"Gravemodell (legacy pipeline) — kun for sammenligning",
    tags=f"Parametrisk masseuttak, gravemodell, kkp, legacy",
...
slpk_result, slpk_package_item, slpk_publishing_result = arcpy.management.SharePackage(
    ...
    username="",
    password="",
    summary=f"IFC-modeller fra BB5 (legacy pipeline) — kun for sammenligning",
    tags=f"Parametrisk masseuttak, gravemodell, kkp, legacy",
    credits="SCBM, COWI NORGE AS",
    public="MYGROUPS",
    groups="CORAV",
...
tunnel_result, tunnel_package_item, tunnel_publishing_result = arcpy.management.SharePackage(
    ...
    username="",
    password="",
    summary=f"IFC-modeller fra BB5 tunnel (legacy pipeline) — kun for sammenligning",
    tags=f"Parametrisk masseuttak, gravemodell, kkp, legacy",
    credits="SCBM, COWI NORGE AS",
    public="MYGROUPS",
    groups="CORAV",
...
berg_result, berg_package_item, berg_publishing_result = arcpy.management.SharePackage(
    ...
    username="",
    password="",
    summary=f"IFC-modeller fra BB5 berg (legacy pipeline) — kun for sammenligning",
    tags=f"Parametrisk masseuttak, gravemodell, kkp, legacy",
    credits="SCBM, COWI NORGE AS",
    public="MYGROUPS",
    groups="CORAV",
...
print("")
print("=" * 60)
print("PUBLISH SUMMARY (legacy — comparison only)")
print("=" * 60)
print(f"  AGOL folder:  {AGOL_FOLDER}")
...
print("=" * 60)
print("")
print("To clean up legacy items later, delete the folder")
